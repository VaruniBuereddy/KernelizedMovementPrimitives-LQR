...
class JointStateRecorder(Node):
    def __init__(self, logging_rate_hz=10):
        ...
        # Output file for joint positions and end-effector pose
        self.output_file = os.path.join(os.path.dirname(__file__), 'joint_positions_demo_tst.csv')
        ...
        # # Write CSV header
        # self.csv_writer.writerow(['elapsed_time'] + [f'fr3_joint{i+1}' for i in range(7)] + ['ee_x', 'ee_y', 'ee_z'])

        # Record the start time
        self.start_time = None

        # Store latest received joint positions and end-effector pose
        ...
        self.get_logger().info("Recording joint positions and end-effector pose to: %s" % self.output_file)

    def joint_state_callback(self, msg):
        """ Store latest joint states (without writing to CSV immediately). """
    ...
    def log_data(self):
        ...
        # Prepare row data
        row = [round(elapsed_time, 6)] + list(self.latest_joint_positions) + list(self.latest_ee_pose)
